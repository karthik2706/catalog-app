    ...
    return (
      <div className="space-y-4 sm:space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900">Performance Analytics</h2>
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
        <div className="min-w-0">
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900">Performance Analytics</h2>
          <p className="text-sm sm:text-base text-slate-600">Core Web Vitals and performance metrics over {data.period}</p>
        </div>
        <Button onClick={fetchPerformanceData} variant="outline" size="sm" className="w-full sm:w-auto flex-shrink-0">
          <RefreshCw className="w-4 h-4 mr-2" />
          Refresh
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
        <Card>
          <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between space-y-2 sm:space-y-0 pb-2">
            <CardTitle className="text-xs sm:text-sm font-medium">Largest Contentful Paint</CardTitle>
            <Badge className={getScoreColor(data.scores.lcp)}>
              {getScoreIcon(data.scores.lcp)}
              <span className="ml-1 capitalize text-xs">{data.scores.lcp}</span>
            </Badge>
          </CardHeader>
          <CardContent className="p-3 sm:p-6">
        ...
        <Card>
          <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between space-y-2 sm:space-y-0 pb-2">
            <CardTitle className="text-xs sm:text-sm font-medium">First Input Delay</CardTitle>
            <Badge className={getScoreColor(data.scores.fid)}>
              {getScoreIcon(data.scores.fid)}
              <span className="ml-1 capitalize text-xs">{data.scores.fid}</span>
            </Badge>
          </CardHeader>
          <CardContent className="p-3 sm:p-6">
            <div className="text-xl sm:text-2xl font-bold text-slate-900">
        ...
        <Card>
          <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between space-y-2 sm:space-y-0 pb-2">
            <CardTitle className="text-xs sm:text-sm font-medium">Cumulative Layout Shift</CardTitle>
            <Badge className={getScoreColor(data.scores.cls)}>
              {getScoreIcon(data.scores.cls)}
              <span className="ml-1 capitalize text-xs">{data.scores.cls}</span>
            </Badge>
          </CardHeader>
          <CardContent className="p-3 sm:p-6">
            <div className="text-xl sm:text-2xl font-bold text-slate-900">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 lg:gap-6">
        <Card>
          <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between space-y-2 sm:space-y-0 pb-2">
            <CardTitle className="text-xs sm:text-sm font-medium">First Contentful Paint</CardTitle>
            <Clock className="h-4 w-4 text-slate-600" />
          </CardHeader>
          <CardContent className="p-3 sm:p-6">
            <div className={`text-xl sm:text-2xl font-bold ${getMetricColor(data.averages.fcp || 0, { good: 1800, poor: 3000 })}`}>
        ...
        <Card>
          <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between space-y-2 sm:space-y-0 pb-2">
            <CardTitle className="text-xs sm:text-sm font-medium">Time to First Byte</CardTitle>
            <Globe className="h-4 w-4 text-slate-600" />
          </CardHeader>
          <CardContent className="p-3 sm:p-6">
            <div className={`text-xl sm:text-2xl font-bold ${getMetricColor(data.averages.ttfb || 0, { good: 800, poor: 1800 })}`}>
      ...
      <Card>
        <CardHeader className="p-3 sm:p-6 pb-3">
          <CardTitle className="text-base sm:text-lg">Recent Performance Data</CardTitle>
        </CardHeader>
        <CardContent className="p-3 sm:p-6">
          <div className="space-y-3 sm:space-y-4">
            {data.metrics.slice(0, 5).map((metric, index) => (
              <div key={index} className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between gap-3 sm:gap-0 p-3 bg-slate-50 rounded-lg">
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-4 w-full sm:w-auto">
                  <div className="text-center">
                    <div className="text-[10px] sm:text-xs font-medium">LCP</div>
                    <div className={`text-xs sm:text-sm ${getMetricColor(metric.lcp, { good: 2500, poor: 4000 })}`}>
                      {Math.round(metric.lcp)}ms
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="text-[10px] sm:text-xs font-medium">FID</div>
                    <div className={`text-xs sm:text-sm ${getMetricColor(metric.fid, { good: 100, poor: 300 })}`}>
                      {Math.round(metric.fid)}ms
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="text-[10px] sm:text-xs font-medium">CLS</div>
                    <div className={`text-xs sm:text-sm ${getMetricColor(metric.cls, { good: 0.1, poor: 0.25 })}`}>
      ...
      <Card>
        ...
        <CardContent className="p-3 sm:p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            <div>
              <h4 className="text-sm sm:text-base font-medium text-slate-900 mb-2">Data Collection</h4>
              <p className="text-xs sm:text-sm text-slate-600">
                Collected {data.total} performance measurements over {data.period}
              </p>
            </div>
            <div>
              <h4 className="text-sm sm:text-base font-medium text-slate-900 mb-2">Core Web Vitals Status</h4>
              <div className="flex flex-wrap gap-2">
                <Badge className={getScoreColor(data.scores.lcp)}>
                  <span className="text-xs">LCP: {data.scores.lcp}</span>
                </Badge>
                <Badge className={getScoreColor(data.scores.fid)}>
                  <span className="text-xs">FID: {data.scores.fid}</span>
                </Badge>
                <Badge className={getScoreColor(data.scores.cls)}>
                  <span className="text-xs">CLS: {data.scores.cls}</span>
                </Badge>
              </div>
            </div>
          </div>
