 ...
 */
export async function generateVideoThumbnailPlaceholder(originalName: string): Promise<ProcessedFile> {
  try {
 ...
 */
async function createPNGFromRGBA(rgbaBuffer: Buffer, width: number, height: number): Promise<Buffer> {
 ...
 */
function generateSimplePlaceholder(originalName: string): ProcessedFile {
 ...
 */
export async function extractRealVideoThumbnail(
  videoBuffer: Buffer,
  originalName: string
