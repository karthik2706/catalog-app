...
export default function GuestCatalogClient({
  ...
  const [clientInfo, setClientInfo] = useState<any>(initialClientInfo)
  ...
  useEffect(() => {
    ...
    if (categoryChanged || searchChanged || pageChanged) {
      ...
      const newUrl = `/guest/${slug}/catalog${queryString ? `?${queryString}` : ''}`
  ...
  // Initial load effect - only runs once or when slug/token changes
  useEffect(() => {
    if (!token) return
