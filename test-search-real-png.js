...
async function testSearchRealPng() {
  try {
    ...
    console.log('Step 2: Testing search by image with real PNG...');
    
    // Use the real PNG
    if (!fs.existsSync('test-real.png')) {
      console.log('test-real.png not found');
      return;
    }

    const imageBuffer = fs.readFileSync('test-real.png');
    console.log('Using real PNG, size:', imageBuffer.length);

    // Create form data using native FormData (Node.js 18+)
    const formData = new FormData();
    const file = new File([imageBuffer], 'test-real.png', { type: 'image/png' });
